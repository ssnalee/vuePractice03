...
<div class="post">
    ...
    <div @click="$store.commit('addLikes',index)" :class="`${content.filter} post-body`" :style="{ 'backgroundImage' : `url(${content.postImage})`}"></div>
    <div class="post-content">
      <p>{{$store.state.likes[index]}} Likes</p>
      <p><strong>{{content.filter}}</strong> {{content.content}}</p>
